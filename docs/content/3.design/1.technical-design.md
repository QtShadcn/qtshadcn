...
| M3 ✅ | Card / Input / Badge / Switch / Tabs / Dialog |
...
| `IconRegistry` | svg 图标注册（qrc 精选集）+ 远程兜底（lucide CDN 拉取 + 内存/磁盘缓存）+ `dataUrl(name, color)` 返回替换好 currentColor 的 base64 data URL，QML 的 Image 直接用，动态变色 | M4 |
| `Models` | QAbstractListModel 基类，供 Table/Select 数据源 | M5 |

> **偏差说明（M5 落地）：** M5 实际落地 `ShadcnTableModel`（`QAbstractTableModel`，非 spec 原写的 `QAbstractListModel` 基类）——表格为二维数据，`QAbstractTableModel` 原生支持 `columnCount()` + role 映射，更贴合表格场景。
...
| **M3** ✅ | Card / Input / Badge / Switch / Tabs / Dialog | showcase 组合示例（如"新建项目"对话框） |
